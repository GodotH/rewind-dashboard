...
import { JUNK_PREFIXES, parseDetail, parseSummary } from './session-parser'
...
describe('parseSummary', () => {
  ...
  describe('firstUserMessage', () => {
    function userLine(content: unknown, minute: number) {
      return JSON.stringify({
        type: 'user',
    ...
    it('resolves a STRING-typed message.content (371 of 454 real sessions)', async () => {
      const filePath = writeJSONL([userLine('ship the naming fix', 0)])

      const summary = await parseSummary(filePath, 'sid', '/repo', 'repo', 100, MTIME)

      expect(summary!.firstUserMessage).toBe('ship the naming fix')
    })

    it('skips a <local-command-caveat> turn and uses the REAL next turn', async () => {
      const filePath = writeJSONL([
        userLine(
          '<local-command-caveat>Caveat: The messages below were generated by the user while running a local command...',
          0,
        ),
        userLine('what broke the session list?', 1),
      ])

      const summary = await parseSummary(filePath, 'sid', '/repo', 'repo', 100, MTIME)

      expect(summary!.firstUserMessage).toBe('what broke the session list?')
    })

    // Derived from the exported constant so a newly added prefix cannot ship untested.
    it.each(JUNK_PREFIXES)('skips injected turn starting with %j so the next real turn wins', async (prefix) => {
      const junk = `${prefix} injected boilerplate the user never typed`
      const filePath = writeJSONL([userLine(junk, 0), userLine('the real prompt', 1)])

      const summary = await parseSummary(filePath, 'sid', '/repo', 'repo', 100, MTIME)

      expect(summary!.firstUserMessage).toBe('the real prompt')
    })

    it('still resolves the first text block of ARRAY-shaped content', async () => {
      const filePath = writeJSONL([
        userLine([{ type: 'tool_result', tool_use_id: 'x', content: 'noise' }, { type: 'text', text: 'array prompt' }], 0),
      ])

      const summary = await parseSummary(filePath, 'sid', '/repo', 'repo', 100, MTIME)

      expect(summary!.firstUserMessage).toBe('array prompt')
    })

    it('yields null when EVERY user turn is injected boilerplate', async () => {
      const filePath = writeJSONL([
        userLine('<local-command-caveat>Caveat: The messages below were generated by the user while running a local command...', 0),
        userLine('<system-reminder>nothing to see</system-reminder>', 1),
        userLine([{ type: 'text', text: '   ' }], 2),
      ])
    ...
    it('truncates a long string prompt to 120 characters', async () => {
      const filePath = writeJSONL([userLine('x'.repeat(300), 0)])
...
describe('parseDetail claudeName', () => {
  let tempDir: string

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'test-detail-title-'))
  })
  ...
  it('captures a custom-title record that used to be silently dropped', async () => {
    const filePath = writeJSONL([
      JSON.stringify({ type: 'custom-title', customTitle: 'F17' }),
      JSON.stringify({ type: 'user', timestamp: '2026-01-01T10:00:00Z', message: { content: [{ type: 'text', text: 'hello' }] } }),
    ])

    const detail = await parseDetail(filePath, 'sid', '/repo', 'repo')

    expect(detail.claudeName).toBe('F17')
    // The custom-title line is metadata, never a turn.
    expect(detail.turns).toHaveLength(1)
  })

  it('keeps the LAST custom-title when several are present', async () => {
    const filePath = writeJSONL([
      JSON.stringify({ type: 'custom-title', customTitle: 'first name' }),
      JSON.stringify({ type: 'user', timestamp: '2026-01-01T10:00:00Z', message: { content: [{ type: 'text', text: 'hello' }] } }),
      JSON.stringify({ type: 'custom-title', customTitle: 'F17' }),
  ...
  it('is null when the session was never renamed', async () => {
    const filePath = writeJSONL([
      JSON.stringify({ type: 'user', timestamp: '2026-01-01T10:00:00Z', message: { content: [{ type: 'text', text: 'hello' }] } }),
    ])
    ...
    expect(detail.claudeName).toBeNull()
  })
})

describe('parseDetail firstUserMessage', () => {
  let tempDir: string

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'test-detail-first-user-'))
  })
  ...
  it('resolves a STRING-typed message.content (363 of 445 real sessions)', async () => {
    const filePath = writeJSONL([userLine('ship the naming fix', 0)])

    const detail = await parseDetail(filePath, 'sid', '/repo', 'repo')

    expect(detail.firstUserMessage).toBe('ship the naming fix')
  })

  it('skips a <local-command-caveat> turn and uses the REAL next turn', async () => {
    const filePath = writeJSONL([userLine(CAVEAT, 0), userLine('what broke the session list?', 1)])

    const detail = await parseDetail(filePath, 'sid', '/repo', 'repo')

    expect(detail.firstUserMessage).toBe('what broke the session list?')
  })

  it('is null when every user turn is injected boilerplate', async () => {
    const filePath = writeJSONL([userLine(CAVEAT, 0), userLine('<system-reminder>nope</system-reminder>', 1)])

    const detail = await parseDetail(filePath, 'sid', '/repo', 'repo')

    expect(detail.firstUserMessage).toBeNull()
  })

  it('resolves the first text block of ARRAY-shaped content', async () => {
    const filePath = writeJSONL([
      userLine([{ type: 'tool_result', tool_use_id: 'x', content: 'noise' }, { type: 'text', text: 'array prompt' }], 0),
    ])
    ...
    expect(detail.firstUserMessage).toBe('array prompt')
  })

  it('PARITY: list and detail resolve an IDENTICAL excerpt for the same file', async () => {
    const filePath = writeJSONL([
      userLine(CAVEAT, 0),
      userLine('why does the sessions list disagree with the detail page?', 1),
      userLine('a later turn that must not win', 2),
    ])

    const summary = await parseSummary(filePath, 'sid', '/repo', 'repo', 100, Date.UTC(2026, 0, 15))
    const detail = await parseDetail(filePath, 'sid', '/repo', 'repo')

    expect(summary!.firstUserMessage).toBe('why does the sessions list disagree with the detail page?')
    expect(detail.firstUserMessage).toBe(summary!.firstUserMessage)
  })
})
