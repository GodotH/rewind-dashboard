...
export function SessionCard({ session, metadata, projectMeta }: SessionCardProps) {
  ...
  return (
    <Link
      ...
      search={{ project: session.projectPath, provider }}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          {provider === 'claude' && <LaunchButton sessionId={session.sessionId} cwd={session.projectPath} />}
          <OverflowMenu
      ...
      <div className="mt-3 flex items-center gap-4 text-xs text-gray-400">
        ...
        {session.model && (
          <span title="Model" className="truncate font-mono text-gray-500">
            {session.model.replace(/^(claude|gemini)-/, '').split('-202')[0]}
          </span>
        )}
